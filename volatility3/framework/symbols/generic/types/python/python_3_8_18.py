...
class Python_3_8_18_IntermedSymbols(intermed.IntermediateSymbolTable):
    ...
    - Developed for PyTorch 3.8.18 on x64 architectures
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.set_type_class("PySetObject", PySetObject)              # https://github.com/python/cpython/blob/3.8/Include/setobject.h
...
class PyDictObject(objects.StructType):
    def create_dict(self, keys, values):
        ...
        if len(keys) != len(values):
            if len(keys) > len(values):
                print("WARNING: Key count exceeds value count... See below!!!\n")
                print(f"Keys ({len(keys)}): " + str(keys) + "\n")
                print(f"Values ({len(values)}):  " + str(values) + "\n")
                dict = {}
                ...
                print("WARNING: Val count exceeds key count... See below!!!\n")
                print(f"Keys ({len(keys)}): " + str(keys) + "\n")
                print(f"Values ({len(values)}):  " + str(values) + "\n")
                dict = {}
...
class PySetObject(objects.StructType):
    def get_value(self):
        """
        Retrieves the variable-length Python list.
        """
        ...
        while i != self.used:
            addr = int.from_bytes(
                curr_layer.read(data_offset + i*16, 8, pad=False),
                byteorder='little'
            )
            if addr != 0x0:
                addresses.append(addr)
        ...
        objs = create_objects(symbol_table_name, self._context, self.vol.layer_name, addresses)
